...
export function InspectorPanel({ findings }: { findings: Finding[] }) {
        ...
        {findings.length === 0 ? (
          ...
          <ul className="flex flex-col gap-1">
            {findings.map((finding, index) => (
              <li
                key={`${finding.ruleId}-${finding.elementId ?? index}`}
                className="flex gap-2 rounded-[6px] px-2 py-2 hover:bg-elevated"
              >
                <span
                  className={cn(
