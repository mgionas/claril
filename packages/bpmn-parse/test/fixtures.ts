/** BPMN 2.0 XML fixtures for the headless parser tests. */

/** Well-formed: start -> task -> end. Mirrors apps/web default-diagram. */
...
/**
 * Known-bad: no end event, and a task ("Orphan") unreachable from the start,
 * plus an unlabeled exclusive gateway that splits. Designed to trigger
 * error-severity findings (so the CLI must exit non-zero).
 */
...
/** A collaboration with a participant + lanes wrapping a process. */
export const collaborationXml = `<?xml version="1.0" encoding="UTF-8"?>
<bpmn:definitions xmlns:bpmn="http://www.omg.org/spec/BPMN/20100524/MODEL"
  id="Definitions_collab" targetNamespace="http://claril.dev/bpmn">
  <bpmn:collaboration id="Collaboration_1">
    <bpmn:participant id="Participant_1" name="Team" processRef="Process_c" />
  </bpmn:collaboration>
  <bpmn:process id="Process_c" isExecutable="false">
    ...
    <bpmn:startEvent id="Start_c" name="Begin" />
    <bpmn:userTask id="Task_c" name="Handle" />
    <bpmn:endEvent id="End_c" name="Finish" />
    <bpmn:sequenceFlow id="Fc1" sourceRef="Start_c" targetRef="Task_c" />
    <bpmn:sequenceFlow id="Fc2" sourceRef="Task_c" targetRef="End_c" />
  </bpmn:process>
</bpmn:definitions>`;

/** Not BPMN at all (no definitions root). */
