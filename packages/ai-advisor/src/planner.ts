...
WHAT POOLS & LANES MEAN (BPMN):
- A POOL (participant) represents ONE independent party/organization/system that runs its own process — e.g. "Customer", "Billing Service", "Bank". A diagram with one pool models a single party's process. Pools communicate ONLY via message flows; sequence flows never cross a pool boundary.
- A LANE is a subdivision INSIDE one pool — a role, team, or system within that same party (e.g. "Frontend" and "Backend" lanes inside an "App" pool). Lanes share the pool's single sequence flow; work moves between lanes with ordinary sequence flows.
- Therefore: adding/renaming/reordering ordinary steps, decisions, events, and data lives INSIDE the existing pool. A request needs a NEW pool only when it introduces a genuinely separate party that exchanges messages with this one — not merely because it mentions another team, role, or system (that's a lane).
...
- Use setDocumentation to add/replace an element's description text (the user asking to "document" or "describe" an element).
...
- You can ONLY use the operations defined in OUTPUT FORMAT below, and updateElement changes an element's NAME only — it cannot move, reparent, restyle, or reconfigure anything. If the request needs an operation that isn't available (e.g. set a user task's assignee/candidate groups; bind an element to a catalog asset; simulate or execute the process), DO NOT emit no-op or unrelated ops. Instead return {"summary": "<one line: what isn't supported yet + the closest supported alternative>", "ops": []}.
- EXPLAIN, DON'T SILENTLY REFUSE: if the request cannot be satisfied within the current pool/lane structure — e.g. it asks to interact with a party that has no pool, or it only makes sense as a separate pool/participant with message flows (which you must not create unprompted) — return {"summary": "<one line: why it doesn't fit the current single-pool process + what IS possible, e.g. 'model it as a lane/step here, or explicitly ask to add a <name> pool'>", "ops": []}. The summary is shown to the user as the AI's reply, so make it a clear, concrete explanation — never an empty or generic message.
- Data objects/stores and text annotations are ARTIFACTS: create with addArtifact, then link to an activity (or, for an annotation, any element) with associate — never connect them with sequence flows.
...
- {"kind":"addArtifact","tempId":string,"artifact":"dataObject"|"dataStore"|"textAnnotation","name"?:string,"text"?:string}  // data object/store, or a text annotation (use "text" for the note body)
- {"kind":"associate","fromRef":string,"toRef":string}  // link a data object/store to an activity, or a text annotation to any element (NOT a sequence flow)
...
- {"kind":"setDocumentation","elementId":string,"text":string}  // set an element's documentation/description text
