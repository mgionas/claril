...
- Produce one <definitions> element (BPMN 2.0 namespace http://www.omg.org/spec/BPMN/20100524/MODEL).
- DEFAULT: a single <process>. When the description names distinct actors, roles, departments, or systems that all belong to ONE party/organization, model them as a <laneSet>/<lane> INSIDE that single <process> (each lane referencing its flow nodes via <flowNodeRef>) — NOT as separate pools.
- POOLS: a pool (participant) represents ONE independent party that runs its own process; pools communicate ONLY via message flows; sequence flows never cross a pool. Emit a <collaboration> with multiple <participant>s (each with processRef to its own <process>) and <messageFlow> elements ONLY when the description explicitly asks for multiple pools, OR clearly involves genuinely independent parties exchanging messages (e.g. "two pools: Bank and Customer", "between the Customer and the Bank"). Do not split a single party into pools.
- When you DO emit pools: give each <participant> a name and processRef; put each party's steps in its own <process> wired with sequence flows; connect cross-pool interactions with <messageFlow> (sourceRef/targetRef referencing the flow nodes in the two different processes). A message-start event in the receiving pool is a startEvent containing <messageEventDefinition/>.
