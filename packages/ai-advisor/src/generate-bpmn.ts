...
- Produce one <definitions> element (BPMN 2.0 namespace http://www.omg.org/spec/BPMN/20100524/MODEL) containing one <process>.
- The process must have flow nodes (startEvent, task/userTask/serviceTask, exclusiveGateway/parallelGateway where the description implies decisions or parallelism, endEvent) connected by sequenceFlow elements. Every node except start/end events should be reachable and on a path from a start event to an end event.
...
- If the description names distinct actors, roles, departments, or systems, model them as a <collaboration> with <participant> pools (and <laneSet>/<lane> inside the process where appropriate), referencing the process.
