...
| W8 | Guided provider-connect wizard — **iteration 1 built (branch)**. Enriched `ProviderMeta` (per-provider description, how-to steps, key prefix/placeholder, note) + soft `keyLooksValid` format check; AI-settings wizard now shows a one-line provider description, numbered how-to steps + console link, a format placeholder, and a non-blocking "doesn't look like a {provider} key" warning. No schema/save change. Spec/plan `docs/superpowers/{specs,plans}/2026-06-21-w8-guided-provider-connect*`. *Queued: multi-provider switching, AI Gateway, Vertex OAuth.* |
...
2. **W8 — Provider-connect wizard.** Iteration 1 (**guided per-provider instructions**) **built on branch** — pending live verify + deploy. Queued for later iterations: multi-provider switching, AI Gateway, Google OAuth→Vertex. *Note: consumer chat subs (ChatGPT/Claude/Gemini plans) can't power third-party API inference — use AI Gateway / BYOK / Vertex OAuth.*
