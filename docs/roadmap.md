...
| W15 | Management surfaces on shadcn — **in progress (branch)**. Split Dashboard (stats overview at `/`) from Projects (`/projects` listing, project cards + inner diagram table); shadcn `card`/`table`/`chart`; dashboard stat cards + AI-usage + recent diagrams + a chart (`getDashboardStats`, pure aggregation unit-tested); Catalog + Settings re-based on shadcn Card/Table. Spec/plan `docs/superpowers/{specs,plans}/2026-06-21-w15-*`. |
