...
| W13 | Personal/Org tenancy — **Phases 1 + 2 deployed**. Model: **Personal** (separate user-scoped subsystem, solo, own BYOK AI, no catalog/unified-knowledge) vs **Organization** (members owner/admin/member; unified knowledge = shared Asset Catalog + AI grounding + impact + templates; **Workspaces** = "directions" with explicit-grant access + workspace roles admin/editor/viewer; org-shared AI). **P1 foundation:** migration `0009` adds `personal_project`, `user_ai_connection`/`user_ai_default`, `diagram.personalProjectId` (XOR with nullable `projectId`, DB CHECK); personal AI resolver reusing the pure W8 core + `getAiConfig(ctx)`; pure `diagramParent` + `assertPersonalProjectAccess` + discriminated `assertDiagramAccess`. **P2 switcher + personal dashboard:** active-context (`getActiveContext` via session `activeOrganizationId`) vs per-diagram context (`diagramContext`); top-bar **context switcher** (Personal / orgs / + Create organization) + org creation w/ default workspace; context-aware dashboard (personal_project flat vs org projects); AI + catalog re-scoped (open diagram uses its own context — no cross-context leak); catalog hidden in Personal; **legacy auto-"Personal" orgs migrated to personal space** (idempotent transactional routine, run on prod). Personal AI off until P3 settings UI. Specs/plans `docs/superpowers/{specs,plans}/2026-06-21-w13-*`. *Queued: P3 personal AI settings UI + catalog-gating polish; P4 org workspaces UI + `workspaceRole` enum formalization.* |
