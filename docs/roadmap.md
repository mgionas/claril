...
2. **W8 — Provider-connect wizard.** Iteration 1 (**guided per-provider instructions**) **deployed to prod** — pending live verify. Queued for later iterations: multi-provider switching, AI Gateway, Google OAuth→Vertex. *Note: consumer chat subs (ChatGPT/Claude/Gemini plans) can't power third-party API inference — use AI Gateway / BYOK / Vertex OAuth.*
